...
def get_risk_factor_indices(mapped_risk_ids: List[List[str]], risk_factor: str, level: int) -> List[int]:
    """Get the indices for a risk factor from mapped_risk_ids.csv."""
    logging.info(f"Getting risk factor indices for: {risk_factor}, Level: {level}")
    indices = []
    for row in mapped_risk_ids:
        if row[1] == risk_factor and int(row[2]) == level:
...
def process_risk_factors(nc_data: List[List[str]], risk_factors: List[Dict], mapped_risk_ids: List[List[str]]) -> List[List[str]]:
    ...
    for rf in risk_factors:
        ...
        indices = get_risk_factor_indices(mapped_risk_ids, risk_factor, rf["level"])
...
def get_highest_risk_factor_level(risk_factors: List[Dict]) -> int:
    """Get the highest level from the risk factor interventions."""
    return max((rf.get("level", 0) for rf in risk_factors), default=1)

def process_risk_target_level(nc_data: List[List[str]], risk_factors: List[Dict]) -> List[List[str]]:
...
def process_nc_file(nc_data: List[List[str]], config: Dict, constants_lookup: Dict[str, Dict[str, str]], mapped_risk_ids: List[List[str]]) -> List[List[str]]:
    ...
    for i, row in enumerate(nc_data):
        ...
        if row[0] == "<Treatment Association>" or row[0] == "<Prevention Association>":
            ...
            logging.info(f"Found a Tax Record block: {row[0]}")
            nc_data = process_tax_interventions(config["risk factors"], nc_data, row[0])
...
def process_tax_interventions(risk_factors: List[Dict], nc_data: List[List[str]], tax_record: str) -> List[List[str]]:
    """Process tax interventions by adjusting retail prices based on target_price_scalar."""
    logging.info(f"Processing tax intervention for: {tax_record}")

    # Get the intervention corresponding to the tax record
    intervention = TAX_MAPPINGS.get(tax_record, "")
    ...
    # Get the first non-empty value in the prices row
    first_non_empty_index = next((idx for idx, val in enumerate(prices) if val and val != ""), None)
    if first_non_empty_index is None:
        logging.warning(f"No non-empty price values found in tax block for: {tax_record}")
        return nc_data

    first_price_value = float(prices[first_non_empty_index])
    if first_price_value == 0.0:
        # Set to baseline price from config
        first_price_value = matching_rf["baseline_price"]
        prices[first_non_empty_index] = str(first_price_value)

    target_price = first_price_value * matching_rf["target_price_scalar"]
    updated_prices = update_price_with_scaling(
        prices,
        first_price_value,
        target_price,
    ...
    logging.info(f"  Baseline Price: {first_price_value:.2f}, Target Price: {target_price:.2f}")
...
def update_price_with_scaling(prices: List[str], baseline_price: float, target_price: float, start_index: int, stop_index: int) -> List[str]:
    """Update prices with scaling from baseline to target price."""
    logging.info(f"Updating prices with scaling from {baseline_price:.2f} to {target_price:.2f}")
    ...
    for i in range(len(prices)):
        ...
        if i <= start_index:
            ...
            progress = (i - start_index) / (stop_index - start_index)
            price = baseline_price + (target_price - baseline_price) * progress
        updated_prices[i] = f"{price:.2f}"
